...
const Categories = () => {
  ...
  const [editColor, setEditColor] = useState<string>(ColorPalette.green);
  ...
  useEffect(() => {
    setEditColor(
      user.categories.find((cat) => cat.id === selectedCategoryId)?.color || ColorPalette.green
    );
  ...
  const handleEditDimiss = () => {
    ...
    setEditColor(ColorPalette.green);
    setEditName("");
